...
public class Main {
    ...
    public static void main(String[] args) throws IOException {
        ...
        var scanner = new Scanner(FileUtility.readFile(new File(programFile)), tokenDefinitionList);
        ...
        if (Objects.nonNull(tokenOutputFile)) {
            try (var writer = new BufferedWriter(new FileWriter(tokenOutputFile))) {
                for (var token : tokens) {
                    writer.write(String.format("%s %s%n", token.getType(), token.getValue()));
                }
            }
        } else {
            tokens.forEach(token -> System.out.printf("%s %s%n", token.getType(), token.getValue()));
        }
    }
